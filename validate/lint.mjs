...
for (const f of files.filter(isComponentCSS)) {
  ...
  for (const r of css.match(/border-radius:\s*([^;]+);/g) || []) {
    checks++;
    if (!/var\(--lk-radius\)|:\s*0(px)?\s*;|50%/.test(r))
      fail(`${f}: border-radius not via --lk-radius -> ${r.trim()}`);
  }
}
...
for (const f of files) {
  ...
  for (const s of css.match(/box-shadow:\s*([^;]+);/g) || []) {
    ...
    const blur = nums[2];
    if (blur && blur !== '0px')
      fail(`${f}: box-shadow has blur (${blur}); only a hard offset is allowed -> ${val.trim()}`);
  }
}
...
for (const cls of FOCUS_REQUIRED) {
  checks++;
  if (!new RegExp(`\\.${cls}[^{]*:focus-visible`).test(allCSS))
    fail(`no :focus-visible rule found for .${cls}`);
}

console.log(`\nLINT · ${checks - errors}/${checks} checks passing.`);
if (errors) {
  console.error(`FAILED: ${errors} design-rule violation(s).`);
  process.exit(1);
}
